...
RUN wget -P /var/tmp/tomcat http://archive.apache.org/dist/tomcat/tomcat-8/v8.0.37/bin/apache-tomcat-8.0.37.tar.gz
RUN tar xzf /var/tmp/tomcat/apache-tomcat-8.0.37.tar.gz -C /var/tmp/tomcat
RUN rm -rf /var/tmp/tomcat/apache-tomcat-8.0.37.tar.gz 
...
ADD ./ /var/tmp/webapp
RUN cd /var/tmp/webapp && mvn package -Dmaven.test.skip=true && cp /var/tmp/webapp/target/CIJD.war /var/tmp/tomcat/apache-tomcat-8.0.37/webapps
